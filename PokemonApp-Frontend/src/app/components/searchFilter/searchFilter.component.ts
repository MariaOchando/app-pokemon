...
export class SearchFilterComponent implements OnInit {
  ...
  async filterPokemon() {
    await this.searchfilterService.getAllPokemons().subscribe((response) => {
      this.pokemonArray = response.results.filter(
        (pokemon: Pokemon) => pokemon.name === this.searchText
      );
  ...
  async filterAbility() {
    await this.searchfilterService.getAllAbilities().subscribe((response) => {
      this.abilityArray = response.results.filter(
        (ability: Ability) => ability.name === this.searchText
      );
  ...
  async filterType() {
    await this.searchfilterService.getAllTypes().subscribe((response) => {
      this.typeArray = response.results.filter(
        (type: Type) => type.name === this.searchText
      );
